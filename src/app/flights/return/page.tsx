  ...
  const handleAddToCart = async (flightResult: any) => {
    ...
    try {
      ...
      if (cartData.cartItem) {
        ...
        const hasOutboundFlight = cartData.cartItem.outboundFlights && cartData.cartItem.outboundFlights.length > 0;
        const isRoundTrip = tripType === "round-trip";

        // Debug information
        console.log("Trip Type:", tripType);
        console.log("Is Round Trip:", isRoundTrip);
        console.log("Has Outbound Flight:", hasOutboundFlight);
        console.log("Cart Data:", cartData.cartItem);

        // Show a success toast
        toast.success("Return flight added to cart!");

        // For round-trip bookings, show hotel suggestion or proceed to booking
        if (isRoundTrip && hasOutboundFlight) {
          console.log("Setting up round-trip completion dialog");
          
          if (!hasHotel) {
            // If no hotel in cart, show hotel suggestion dialog
            const city = flightResult.flights[0].origin.city;
            const date = flightResult.flights[0].departureTime;

            setNextStepsOptions({
              title: "Round-trip flight added to cart!",
              description: `Would you like to find a hotel in ${city} or proceed to booking?`,
            ...
            setNextStepsOptions({
              title: "Round-trip flight added to cart!",
              description: "Your round-trip flight has been added to your cart. Would you like to proceed to booking or continue shopping?",
              hotelUrl: "",
              flightUrl: "",
              hotelButtonText: "",
              flightButtonText: "",
        ...
        if (!isRoundTrip) {
          if (!hasHotel) {
            ...
            setNextStepsOptions({
              title: "Return flight added to cart!",
              description: `Would you like to find a hotel in ${city} or proceed to booking?`,
              hotelUrl: `/hotels?city=${encodeURIComponent(city)}&checkInDate=${encodeURIComponent(new Date(date).toISOString().split("T")[0])}&checkOutDate=${encodeURIComponent(new Date(new Date(date).getTime() + 24 * 60 * 60 * 1000).toISOString().split("T")[0])}`,
              flightUrl: "",
              hotelButtonText: `Find a hotel in ${city}`,
            ...
            setNextStepsOptions({
              title: "Return flight added to cart!",
              description:
                "Your return flight has been added to your cart. Would you like to proceed to booking or continue shopping?",
              hotelUrl: "",
              flightUrl: "",
              hotelButtonText: "",
              flightButtonText: "",
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              {(!isRoundTrip || (isRoundTrip && !nextStepsOptions?.hotelButtonText) || (isRoundTrip && nextStepsOptions?.hotelButtonText)) && (
                <>
                  <Button
                    onClick={() => {
